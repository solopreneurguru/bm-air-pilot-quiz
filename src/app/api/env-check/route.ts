...
export async function GET() {
    const {
        ...
        AIRTABLE_TABLE_NAME,
        AIRTABLE_TABLE_ID,
        AIRTABLE_SCANS_TABLE_ID,
        AIRTABLE_SCANS_TABLE_NAME,
    } = process.env as Record<string, string>;

    const tablePath = encodeURIComponent(AIRTABLE_TABLE_ID || AIRTABLE_TABLE_NAME || "");
    const scansPath = encodeURIComponent(AIRTABLE_SCANS_TABLE_ID || AIRTABLE_SCANS_TABLE_NAME || "");
    ...
    return NextResponse.json({
        ...
        scansTableName: AIRTABLE_SCANS_TABLE_NAME || null,
        scansPath,
    });
} 